...
from .models import Employee, Department, Designation, EmergencyContact, EmployeeDocument, Device, DeviceAllocation, DeviceRequest, PublicHoliday, LeaveType, LeaveApplication, UserProfile
...
class DeviceCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_form(self, form_class=None):
        ...
        # Add placeholder and CSS class to purchase_date field
...
class DeviceRequestCreateView(LoginRequiredMixin, CreateView):

    model = DeviceRequest

    template_name = 'devices/device_request_form.html'

    fields = ['priority', 'required_date', 'reason']
    ...
    def get_form(self, form_class=None):
        ...
        # Add device field manually for available devices

        form.fields['device'] = forms.ModelChoiceField(
        ...
        # Add CSS classes to priority field
        ...
        # Add CSS classes and placeholder to required_date field with date input type

        form.fields['required_date'].widget = forms.DateInput(attrs={

            'class': 'form-control input modern-form-control',
        ...
        # Add CSS classes to reason field
    ...
    def form_valid(self, form):
        ...
        form.instance.employee = self.request.user.profile.employee

        # Set device to None initially - will be allocated later

        form.instance.device = None

        messages.success(self.request, 'Device request submitted successfully!')
    ...
    def get_success_url(self):
        ...
        return reverse_lazy('employees:my_profile')
...
def asset_management(request):
    ...
    # Only allow superusers and staff to access asset management
    ...
    if not request.user.is_superuser and not request.user.is_staff:
        ...
        messages.error(request, 'You do not have permission to access asset management.')
    ...
    pending_requests = DeviceRequest.objects.filter(status='pending').order_by('-request_date')



    approved_requests = DeviceRequest.objects.filter(status='approved').order_by('-approved_date')



    allocated_requests = DeviceRequest.objects.filter(status='allocated').order_by('-allocated_date')



    return_requests = DeviceRequest.objects.filter(return_requested_date__isnull=False, status__in=['approved', 'allocated']).order_by('-return_requested_date')
    ...
    return render(request, 'devices/asset_management.html', context)
...
def device_request_details(request, request_id):



    # Only allow superusers and staff to view request details
    ...
    data = {
        ...
        'employee_name': device_request.employee.full_name,



        'employee_code': device_request.employee.employee_code,



        'department': device_request.employee.department.name,



        'device_type': device_request.get_device_type_display(),



        'priority': device_request.get_priority_display(),



        'required_date': device_request.required_date.strftime('%b %d, %Y'),



        'request_date': device_request.request_date.strftime('%b %d, %Y'),
...
def approve_device_request(request, request_id):
    ...
    # Only allow superusers and staff to approve requests
    ...
    try:
        ...
        device_request.approve_request(request.user)
        ...
        Notification.objects.create(
            ...
            message=f'Your device request for {device_request.get_device_type_display()} has been approved.',
            ...
            target_users=[device_request.employee.user_profile.user]
...
def reject_device_request(request, request_id):
    ...
    # Only allow superusers and staff to reject requests
    ...
    try:
        ...
        device_request.rejection_reason = reason
        ...
        device_request.approved_date = timezone.now()
        ...
        Notification.objects.create(
            ...
            message=f'Your device request for {device_request.get_device_type_display()} has been rejected. Reason: {reason}',
...
def get_available_devices(request, request_id):



    # Only allow superusers and staff to view available devices
    ...
    # Get available devices of the requested type
...
def allocate_device_to_request(request, request_id):
    ...
    try:
        ...
        if device.status != 'available':
            ...
            return JsonResponse({'error': 'Device is not available'}, status=400)
        ...
        device_request.allocate_device(request.user, device)
        ...
        Notification.objects.create(
            ...
            message=f'Your requested device ({device.device_name}) has been allocated to you.',
...
def request_device_return(request, request_id):



    # Only allow the employee who made the request to request return
    ...
    if request.user.profile.employee != device_request.employee:
    ...
    if not device_request.can_be_returned:



        return JsonResponse({'error': 'Device cannot be returned'}, status=400)
    ...
    try:
        ...
        # Create notification for staff/superuser
        ...
        Notification.objects.create(
            ...
            message=f'{device_request.employee.full_name} has requested to return their device ({device_request.device.device_name}).',
...
def approve_device_return(request, request_id):



    # Only allow superusers and staff to approve returns
    ...
    try:
        ...
        device_request.approve_return(request.user, notes)
        ...
        Notification.objects.create(
            ...
            title='Device Return Approved',



            message=f'Your device return request has been approved. Please return the device to IT department.',
